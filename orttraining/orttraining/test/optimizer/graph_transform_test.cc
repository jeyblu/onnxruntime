...
std::vector<std::vector<int64_t>> input_dims) {
  auto model_file = model_path + ".onnx";
  const PathString model_uri = ORT_TSTR(model_file);
  ...
  for (auto i = 0; i < total_rank; i++) {
    ...
    model_file = model_path + "_partition_rank_" + std::to_string(i) + ".onnx";
    auto model_uri2 = ORT_TSTR(model_file);
  ...
  ORT_ENFORCE(ret.IsOK());
  model_file = model_path + "_partition_combine.onnx";
  auto model_uri2 = ORT_TSTR(model_file);
